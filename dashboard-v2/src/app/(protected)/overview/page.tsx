...
const PROTO_COLORS = ['#6366f1', '#06b6d4', '#10b981', '#f59e0b', '#f43f5e', '#8b5cf6', '#ec4899']
...
function Empty({ text }: { text: string }) {
  return <div className="flex items-center justify-center py-8 text-zinc-600 text-sm">{text}</div>
}
    ...
    <div className={cn('rounded-lg border p-4 flex items-center gap-6', cfg.border, cfg.bg)}>
      ...
      <div className="hidden md:flex items-center gap-5 flex-shrink-0 text-center">
        {[
          ...
          { label: 'Anomali 24s',    value: data.anomalies_24h,    href: '/anomaly',     color: data.anomalies_24h    > 0 ? 'text-yellow-400' : 'text-zinc-400' },
...
function KpiCard({ label, value, sub, color, href }: {
  ...
  const inner = (
    <div className="bg-[#13161e] border border-white/[0.06] rounded-lg px-4 py-3 hover:border-white/10 transition-colors group">
      <p className="text-[11px] text-zinc-500 uppercase tracking-wide mb-1 truncate">{label}</p>
...
function AttackChainKpis() {
  ...
  return (
    <>
      <KpiCard label="Aktif Saldırı IP"  value={data?.active_ips    ?? '—'} color={data && data.active_ips   > 0 ? 'text-red-400'    : 'text-zinc-300'} href="/timeline" />
      <KpiCard label="Zincir 24s"         value={data?.chains_24h    ?? '—'} color={data && data.chains_24h   > 0 ? 'text-orange-400' : 'text-zinc-300'} href="/timeline" />
      <KpiCard label="Kritik Zincir 24s"  value={data?.critical_24h  ?? '—'} color={data && data.critical_24h > 0 ? 'text-red-400'    : 'text-zinc-300'} href="/timeline" />
    </>
  )
}
...
function IncidentKpis() {
  ...
  return (
    <>
      <KpiCard label="Açık Incident"          value={data?.open          ?? '—'} color={data && data.open          > 0 ? 'text-red-400'    : 'text-zinc-300'} href="/incidents" />
      <KpiCard label="İnceleniyor"            value={data?.investigating  ?? '—'} color={data && data.investigating > 0 ? 'text-yellow-400' : 'text-zinc-300'} href="/incidents" />
...
function MttdRow() {
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      <div className="bg-[#13161e] border border-white/[0.06] rounded-lg p-4 flex items-start gap-3">
        ...
        <div className="min-w-0">
          <p className="text-[11px] text-zinc-500 uppercase tracking-wide">Ort. Tespit Süresi</p>
          <p className="text-2xl font-bold tabular-nums text-blue-400 mt-1">{fmt(data?.overall_mttd_minutes ?? null)}</p>
          <p className="text-[11px] text-zinc-600 mt-0.5">MTTD · Son 7 gün</p>
        </div>
      </div>
      <div className="bg-[#13161e] border border-white/[0.06] rounded-lg p-4 flex items-start gap-3">
        ...
        <div className="min-w-0">
          <p className="text-[11px] text-zinc-500 uppercase tracking-wide">Ort. Yanıt Süresi</p>
          <p className="text-2xl font-bold tabular-nums text-orange-400 mt-1">{fmt(data?.overall_mttr_minutes ?? null)}</p>
          <p className="text-[11px] text-zinc-600 mt-0.5">MTTR · Son 7 gün</p>
        </div>
      </div>
      <div className="bg-[#13161e] border border-white/[0.06] rounded-lg p-4 flex items-start gap-3">
        <div className={cn('w-8 h-8 rounded-md flex items-center justify-center flex-shrink-0', rate === null ? 'bg-zinc-800' : rate >= 80 ? 'bg-emerald-500/10' : 'bg-yellow-500/10')}>
          <CheckCircle2 size={14} className={rate === null ? 'text-zinc-600' : rate >= 80 ? 'text-emerald-400' : 'text-yellow-400'} />
        </div>
        <div className="min-w-0">
          <p className="text-[11px] text-zinc-500 uppercase tracking-wide">Çözüm Oranı</p>
          <p className={cn('text-2xl font-bold tabular-nums mt-1', rate === null ? 'text-zinc-500' : rate >= 80 ? 'text-emerald-400' : 'text-yellow-400')}>
  ...
  return (
    <Link href={`/agents/${agent.agent_id}`} className="flex items-center gap-3 px-4 py-2.5 hover:bg-white/[0.03] transition-colors border-b border-white/[0.04] last:border-0">
      ...
      <div className="flex items-center gap-4">
        <div className="text-right"><p className="text-[10px] text-zinc-600 mb-0.5">CPU</p>{pct(snapshot?.cpu.usage_percent ?? null)}</div>
        <div className="text-right"><p className="text-[10px] text-zinc-600 mb-0.5">RAM</p>{pct(snapshot?.memory.usage_percent ?? null)}</div>
      </div>
    ...
    <Panel title="Başarısız Auth (24s)" icon={Bell} href="/failed-auth">
      <div className="p-4 space-y-3">
        <div className="flex items-end justify-between">
          <div>
            <p className={cn('text-3xl font-bold tabular-nums', (data?.total ?? 0) > 0 ? 'text-orange-400' : 'text-zinc-300')}>
              {data?.total ?? '—'}
            </p>
            <p className="text-[11px] text-zinc-500 mt-0.5">Toplam başarısız giriş</p>
          </div>
          {top && (
            <div className="text-right">
              <p className="text-[10px] text-zinc-500 mb-0.5">En aktif kaynak</p>
              <p className="text-xs font-mono text-orange-300">{top.ip}</p>
              <p className="text-[10px] text-zinc-600">{top.count} deneme</p>
            </div>
          )}
        </div>
        {hasSparkline ? (
          ...
          <div className="h-16 flex items-center justify-center text-zinc-700 text-xs">Yeterli veri yok</div>
        )}
      </div>
    </Panel>
  )
      ...
      <div className="divide-y divide-white/[0.04]">
        {rows.map(r => (
          <div key={r.label} className="flex items-center justify-between px-4 py-2.5">
            ...
            <span className={cn('text-xs font-mono font-semibold', r.alert ? 'text-yellow-400' : 'text-zinc-300')}>{String(r.value)}</span>
          </div>
        ))}
      </div>
    </Panel>
  ...
  return (
    <Panel title="Anomali Tespiti (24s)" icon={Activity} href="/anomaly">
      <div className="p-4 space-y-3">
        <div className="flex items-end justify-between">
          <div>
            <p className={cn('text-3xl font-bold tabular-nums', (data?.total ?? 0) > 0 ? 'text-indigo-400' : 'text-zinc-300')}>
              {data?.total ?? '—'}
            </p>
            <p className="text-[11px] text-zinc-500 mt-0.5">Toplam anomali</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] text-zinc-500 mb-0.5">Etkilenen IP</p>
        ...
        <div className="space-y-2">
          {bars.map(b => (
            <div key={b.label} className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-mono text-zinc-500 w-5 text-right">{b.value}</span>
            </div>
          ))}
        </div>
      </div>
...
function FreshnessChip({ item }: { item: SourceHealthItem }) {
  ...
  return (
    <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-zinc-800/60 border border-white/[0.05]" title={title}>
      <span className={cn('w-1.5 h-1.5 rounded-full flex-shrink-0', dot)} />
      <span className={cn('text-[11px] font-medium', text)}>{label}</span>
      {age !== null && <span className="text-[10px] text-zinc-600">{age < 1 ? '<1dk' : `${age.toFixed(0)}dk`}</span>}
    </div>
  )
}
...
function DataFreshnessBar() {
  ...
  return (
    <div className="bg-[#13161e] border border-white/[0.06] rounded-lg px-4 py-2.5">
      <div className="flex items-center gap-3 flex-wrap">
        <span className="text-[11px] text-zinc-500 uppercase tracking-wide flex-shrink-0">Veri Kaynakları</span>
        {staleCount > 0 && (
...
function TrafficVolumePanel({ data }: { data: TrafficVolumeResponse | undefined }) {
  ...
  const option = {
    ...
    xAxis: {
      type: 'category',
      data: times,
      axisLabel: { color: '#52525b', fontSize: 9, interval: Math.floor(times.length / 4) },
        ...
        <div className="p-3 space-y-2">
          {data && (
            <div className="flex gap-3 mb-3">
              <div className="flex-1 bg-zinc-900 rounded p-2 text-center">
                <p className={cn('text-xl font-bold tabular-nums', data.critical_count > 0 ? 'text-red-400' : 'text-zinc-300')}>{data.critical_count}</p>
                <p className="text-[10px] text-zinc-600 mt-0.5">Kritik</p>
              </div>
              <div className="flex-1 bg-zinc-900 rounded p-2 text-center">
                <p className={cn('text-xl font-bold tabular-nums', data.high_risk_count > 0 ? 'text-orange-400' : 'text-zinc-300')}>{data.high_risk_count}</p>
                <p className="text-[10px] text-zinc-600 mt-0.5">Yüksek Risk</p>
              </div>
              <div className="flex-1 bg-zinc-900 rounded p-2 text-center">
                <p className="text-xl font-bold tabular-nums text-zinc-300">{data.total_alerts}</p>
                <p className="text-[10px] text-zinc-600 mt-0.5">Alert</p>
              </div>
            </div>
          )}
          {top.map(src => (
            <div key={src.ip} className="flex items-center gap-2">
              <span className="font-mono text-[11px] text-zinc-400 w-28 truncate flex-shrink-0" title={src.ip}>{src.ip}</span>
              <div className="flex-1 h-1 bg-zinc-800 rounded-full overflow-hidden">
                <div className="h-full rounded-full bg-red-500" style={{ width: `${src.composite_score}%` }} />
              </div>
              <span className="text-[11px] font-mono text-zinc-500 w-6 text-right">{src.composite_score}</span>
              {src.country_code && (
                <span className="text-[10px] text-zinc-600 w-6 text-right flex-shrink-0">{src.country_code}</span>
              )}
            </div>
          ))}
        </div>
...
function buildAlertTrendOption(data: AlertVolumeResponse) {
  ...
  return {
    ...
    xAxis: {
      ...
      data: times,
      axisLine: { show: false }, axisTick: { show: false },
      axisLabel: { color: '#52525b', fontSize: 10, interval: Math.floor(times.length / 8) },
    },
    yAxis: {
      type: 'value',
      minInterval: 1,
...
function RiskBar({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-1 min-w-0">
      ...
      <span className="text-[11px] font-mono text-zinc-500 w-6 text-right flex-shrink-0">{score.toFixed(0)}</span>
    </div>
  )
}
    ...
    <Panel title="Yüksek Riskli Varlıklar" icon={ShieldAlert} href="/asset-risk">
      {data.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-8 gap-2">
          ...
          <p className="text-zinc-600 text-sm">Risk tespiti yok</p>
        </div>
      ) : (
        <div className="divide-y divide-white/[0.04]">
          {data.map(a => (
            ...
            <div className="divide-y divide-white/[0.04]">
              {(corrData?.events ?? []).slice(0, 6).map((ev) => (
                <div key={ev.corr_id} className="flex items-start gap-3 px-4 py-2.5">
                  <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 mt-1.5 ${SEV_DOT[ev.severity] ?? 'bg-zinc-600'}`} />
                    ...
                    {ev.mitre_tactics.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {ev.mitre_tactics.slice(0, 2).map(t => (
                          <span key={t} className={cn('text-[10px] px-1.5 py-0.5 rounded font-medium', TACTIC_COLORS[t] ?? 'bg-zinc-800 text-zinc-400')}>
                            {t.replace(/-/g, ' ')}
                          </span>
                        ))}
                      </div>
