...
|---|-------|----------|--------------|
| **S1** | **Reboot testi** | Bilgisayarı kapat/aç → GNS3 aç → tüm otomasyonun çalıştığını doğrula. VMware user service, VyOS postconfig script, Alpine autostart test edilmedi. Sorun çıkarsa burada düzelt. | ~15 dk |
| **S2** | **Kali saldırı testleri** | SSH brute force + port scan → kill chain → incident akışı uçtan uca doğrula. Kali (192.168.203.132) hazır. Paramiko + socket script gerekli. Demo için şart. | ~30 dk |
| **S3** | **README belgeleme** | A4-2: Mimari, kurulum, özellikler. Teslim için gerekli, demo sonrası da yapılabilir. | ~20 dk |
